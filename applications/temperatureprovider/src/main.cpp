...
int main(void) {
    ...
    Logger& logger = CreateLogger("MAIN", "TemperatureProvider main", LogLevel::kVerbose);
    ...
    logger.LogInfo() << "TemperatureProvider version" << g_appVersion;

    ara::demo::TemperatureProvider temperatureProvider;
    temperatureProvider.Init();
    std::thread temperatureSensor(&ara::demo::TemperatureProvider::monitorTemperature, &temperatureProvider);
